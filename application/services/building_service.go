...
func (s *BuildingApplicationService) CreateBuildingType(ctx context.Context, req *CreateBuildingTypeRequest) (*CreateBuildingTypeResponse, error) {
	...
	// 转换建筑类别
	category, err := s.parseBuildingCategory(req.Category)
	if err != nil {
		return nil, fmt.Errorf("invalid category: %w", err)
	}
	
	// 创建建筑类型
	buildingType := building.NewBuildingType(req.Name, category, req.MaxLevel)
	...
	// 保存建筑类型
	if err := s.buildingRepo.SaveBuildingType(ctx, buildingType); err != nil {
		return nil, fmt.Errorf("failed to save building type: %w", err)
	}
	
	// 发布事件
	event := building.NewBuildingTypeCreatedEvent(buildingType.GetID(), req.Name, category)
	...
	return &CreateBuildingTypeResponse{
		...
		Category:       buildingType.GetCategory().String(),
		MaxLevel:       buildingType.GetMaxLevel(),
		BaseCost:       buildingType.GetBaseCost(),
		BuildTime:      buildingType.GetBuildTime(),
		IsActive:       buildingType.IsActive(),
		CreatedAt:      buildingType.GetCreatedAt(),
	}, nil
}
...
func (s *BuildingApplicationService) StartConstruction(ctx context.Context, req *StartConstructionRequest) (*StartConstructionResponse, error) {
	...
	// 获取建筑类型
	buildingType, err := s.buildingRepo.FindBuildingTypeByID(ctx, req.BuildingTypeID)
	if err != nil {
	...
	// 检查位置是否可用
	existingBuilding, err := s.buildingRepo.FindByPlayerAndPosition(ctx, req.PlayerID, req.Position)
	if err != nil {
	...
	// 开始建造
	construction, buildingAggregate, err := s.buildingService.StartConstruction(
		ctx,
		req.PlayerID,
		req.BuildingTypeID,
		req.Position,
	...
	// 发布事件
	event := building.NewConstructionStartedEvent(
		construction.GetID(),
		buildingAggregate.GetID(),
		req.PlayerID,
		req.BuildingTypeID,
	)
	if err := s.eventBus.Publish(ctx, event); err != nil {
	...
	return &StartConstructionResponse{
		...
		BuildingID:     buildingAggregate.GetID(),
		PlayerID:       construction.GetPlayerID(),
		BuildingTypeID: construction.GetBuildingTypeID(),
		Position:       construction.GetPosition(),
		SlotID:         construction.GetSlotID(),
...
func (s *BuildingApplicationService) CompleteConstruction(ctx context.Context, req *CompleteConstructionRequest) (*CompleteConstructionResponse, error) {
	...
	// 完成建造
	result, err := s.buildingService.CompleteConstruction(ctx, req.ConstructionID, req.InstantComplete)
	if err != nil {
	...
	// 发布事件
	event := building.NewConstructionCompletedEvent(
		req.ConstructionID,
		result.BuildingID,
...
func (s *BuildingApplicationService) UpgradeBuilding(ctx context.Context, req *UpgradeBuildingRequest) (*UpgradeBuildingResponse, error) {
	...
	// 升级建筑
	result, err := s.buildingService.UpgradeBuilding(ctx, req.BuildingID, req.TargetLevel)
	if err != nil {
		return nil, fmt.Errorf("failed to upgrade building: %w", err)
	}
	
	// 发布事件
	event := building.NewBuildingUpgradeStartedEvent(
		req.BuildingID,
		buildingAggregate.GetPlayerID(),
		oldLevel,
		req.TargetLevel,
	)
	if err := s.eventBus.Publish(ctx, event); err != nil {
...
func (s *BuildingApplicationService) GetPlayerBuildings(ctx context.Context, req *GetPlayerBuildingsRequest) (*GetPlayerBuildingsResponse, error) {
	...
	// 构建查询
	query := building.NewBuildingQuery().WithPlayer(req.PlayerID)
	...
	if req.Status != "" {
		status, err := s.parseBuildingStatus(req.Status)
		if err != nil {
	...
	for i, buildingAggregate := range buildings {
		// 获取建筑类型信息
		...
		buildingResponse := &BuildingResponse{
			BuildingID:     buildingAggregate.GetID(),
			BuildingTypeID: buildingAggregate.GetBuildingTypeID(),
			Level:          buildingAggregate.GetLevel().GetCurrentLevel(),
			Position:       buildingAggregate.GetPosition(),
			SlotID:         buildingAggregate.GetSlotID(),
			Status:         buildingAggregate.GetStatus().String(),
			Effects:        effectResponses,
			Production:     buildingAggregate.GetProduction(),
			CreatedAt:      buildingAggregate.GetCreatedAt(),
			LastUpdated:    buildingAggregate.GetUpdatedAt(),
		}
...
func (s *BuildingApplicationService) parseBuildingCategory(categoryStr string) (building.BuildingCategory, error) {
	switch categoryStr {
	case "residential":
		return building.CategoryResidential, nil
	case "commercial":
		return building.CategoryCommercial, nil
	case "industrial":
		return building.CategoryIndustrial, nil
	case "military":
		return building.CategoryMilitary, nil
	case "decoration":
		return building.CategoryDecoration, nil
	case "special":
		return building.CategorySpecial, nil
	default:
		return building.CategoryResidential, fmt.Errorf("unknown category: %s", categoryStr)
	}
}

// parseBuildingStatus 解析建筑状态
func (s *BuildingApplicationService) parseBuildingStatus(statusStr string) (building.BuildingStatus, error) {
	switch statusStr {
	case "planning":
		return building.StatusPlanning, nil
	case "constructing":
		return building.StatusConstructing, nil
	case "completed":
		return building.StatusCompleted, nil
	case "upgrading":
		return building.StatusUpgrading, nil
	case "demolished":
		return building.StatusDemolished, nil
	default:
		return building.StatusPlanning, fmt.Errorf("unknown status: %s", statusStr)
	}
}