...
// NPCHandler NPC TCP处理�?type NPCHandler struct {
...
// NewNPCHandler 创建NPC处理�?func NewNPCHandler(npcService *services.NPCService, logger logger.Logger) *NPCHandler {
	return &NPCHandler{
...
// RegisterHandlers 注册处理�?func (h *NPCHandler) RegisterHandlers(server network.Server) error {
	// 注册NPC相关处理�?	if err := server.RegisterHandler(&NPCListHandler{h}); err != nil {
...
// NPC列表处理�?type NPCListHandler struct {
	*NPCHandler
}
...
// NPC信息处理�?type NPCInfoHandler struct {
	*NPCHandler
}
...
// NPC交互处理�?type NPCInteractHandler struct {
	*NPCHandler
}
...
// NPC对话处理�?type NPCDialogueHandler struct {
	*NPCHandler
}
...
func (h *NPCDialogueHandler) Handle(ctx context.Context, conn *netcore.Connection, packet *netcore.Packet) error {
	...
	// 调用服务层进行对�?	dialogue, err := h.npcService.StartDialogue(ctx, playerID, npcID, dialogueID)
	if err != nil {
...
// NPC交易处理�?type NPCTradeHandler struct {
	*NPCHandler
}
...
func (h *NPCTradeHandler) Handle(ctx context.Context, conn *netcore.Connection, packet *netcore.Packet) error {
	...
	// 调用服务层进行交�?	result, err := h.npcService.TradeWithNPC(ctx, playerID, npcID, tradeData)
	if err != nil {
...
// NPC任务处理�?type NPCQuestHandler struct {
	*NPCHandler
}
...
func (h *NPCQuestHandler) Handle(ctx context.Context, conn *netcore.Connection, packet *netcore.Packet) error {
	...
	// 调用服务层处理任�?	result, err := h.npcService.HandleQuest(ctx, playerID, npcID, questAction)
	if err != nil {
...
// NPC商店处理�?type NPCShopHandler struct {
	*NPCHandler
}
...
func (h *NPCShopHandler) Handle(ctx context.Context, conn *netcore.Connection, packet *netcore.Packet) error {
	...
	// 调用服务层处理商店操�?	result, err := h.npcService.HandleShop(ctx, playerID, npcID, shopAction, req.Data)
	if err != nil {
...
// NPC战斗处理�?type NPCBattleHandler struct {
	*NPCHandler
}
...
func (h *NPCBattleHandler) Handle(ctx context.Context, conn *netcore.Connection, packet *netcore.Packet) error {
	...
	// 调用服务层处理战�?	result, err := h.npcService.BattleWithNPC(ctx, playerID, npcID, battleAction)
	if err != nil {
...
func (h *NPCStatusHandler) Handle(ctx context.Context, conn *netcore.Connection, packet *netcore.Packet) error {
	...
	// 调用服务层获取NPC状�?	status, err := h.npcService.GetNPCStatus(ctx, npcID)
	if err != nil {
...
// NPC移动处理�?type NPCMoveHandler struct {
	*NPCHandler
}
...
// sendResponse 发送响�?func (h *NPCHandler) sendResponse(conn *netcore.Connection, msgType uint32, response NPCResponse) error {
...
// sendErrorResponse 发送错误响�?func (h *NPCHandler) sendErrorResponse(conn *netcore.Connection, errorMsg string) error {
