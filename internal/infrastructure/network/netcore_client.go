...
type Client interface {
	...
	RegisterHandler(handler ClientMessageHandler) error
	
	// UnregisterHandler 取消注册消息处理器
	UnregisterHandler(messageType uint32) error
	
	// SetConnectionHandler 设置连接处理器
	SetConnectionHandler(handler ClientConnectionHandler)
	...
	// Send 发送消息
	Send(packet *netcore.Packet) error
	
	// SendWithTimeout 带超时的发送消息
	SendWithTimeout(packet *netcore.Packet, timeout time.Duration) error
...
func NewNetcoreClient(config *ClientConfig, logger logger.Logger) Client {
	if config == nil {
		config = &ClientConfig{
	...
	// 创建netcore客户端配置
	netcoreConfig := &netcore.ClientConfig{
	...
	// 创建netcore客户端
	netcoreClient := netcore.NewClient(netcoreConfig)
...
// SetConnectionHandler 设置连接处理器
func (c *NetcoreClient) SetConnectionHandler(handler ClientConnectionHandler) {
...
// Send 发送消息
func (c *NetcoreClient) Send(packet *netcore.Packet) error {
	c.mu.RLock()
	conn := c.conn
	c.mu.RUnlock()
...
// SendWithTimeout 带超时的发送消息
func (c *NetcoreClient) SendWithTimeout(packet *netcore.Packet, timeout time.Duration) error {
...
func (c *NetcoreClient) SendRequest(request *netcore.Packet, timeout time.Duration) (*netcore.Packet, error) {
	...
	// 等待响应（这里需要根据实际协议实现）
	// 简化处理，返回nil表示需要在实际使用中完善
	c.logger.Debug("Request sent, waiting for response", "request_type", request.GetType())
...
// Start 启动客户端
func (c *NetcoreClient) Start(ctx context.Context) error {
...
// Stop 停止客户端
func (c *NetcoreClient) Stop() error {
...
func (c *NetcoreClient) setupNetcoreHandlers() {
	// 连接建立事件
...
func (c *NetcoreClient) startHeartbeat() {
...
// sendHeartbeat 发送心跳
func (c *NetcoreClient) sendHeartbeat() {
	if !c.IsConnected() {
		return
	}
