...
func (cp *ConfigProvider) RegisterServices(container *Container) error {
	...
	container.RegisterSingleton("config", func(c *Container) (interface{}, error) {
		...
		return configLoader.LoadFromFile(envMgr.GetConfigPath())
	...
	container.RegisterSingleton("config.hot_reload", func(c *Container) (interface{}, error) {
		...
		return config.NewHotReloader(&config.HotReloadConfig{
			Enabled:      config.HotReload.Enabled,
			WatchPaths:   config.HotReload.WatchPaths,
			DebounceTime: config.HotReload.DebounceTime,
		}), nil
	}, "config")
...
func (lp *LoggingProvider) RegisterServices(container *Container) error {
	...
	container.RegisterSingleton("logger", func(c *Container) (interface{}, error) {
		...
		case "zap":
			return logging.NewZapLogger(&logConfig)
		case "file":
			return logging.NewFileLogger(&logConfig)
		case "console":
			return logging.NewConsoleLogger(&logConfig)
		default:
			return logging.NewZapLogger(&logConfig)
		}
	}, "config")
