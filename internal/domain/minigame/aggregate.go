...
func (mg *MinigameAggregate) StartGame() error {
	...
	// 发布游戏开始事件
	mg.addEvent(NewGameStartedEvent(mg.ID, mg.GameID, mg.GameType, mg.CreatorID, mg.getPlayerIDs()))
...
func (mg *MinigameAggregate) EndGame(reason GameEndReason) error {
	...
	// 发布游戏结束事件
	mg.addEvent(NewGameEndedEvent(mg.ID, mg.GameID, reason, mg.Duration, mg.Results))
...
func (mg *MinigameAggregate) AddPlayer(playerID uint64, playerName string) error {
	...
	// 添加玩家
	player := NewGamePlayer(playerID, playerName)
	mg.Players = append(mg.Players, player)
	
	// 发布玩家加入事件
	mg.addEvent(NewPlayerJoinedGameEvent(mg.ID, mg.GameID, playerID, playerName))
...
func (mg *MinigameAggregate) RemovePlayer(playerID uint64, reason PlayerLeaveReason) error {
	...
	for i, player := range mg.Players {
		if player.PlayerID == playerID {
			// 更新玩家状态
			player.Status = PlayerStatusLeft
			player.LeaveTime = time.Now()
			...
			// 发布玩家离开事件
			mg.addEvent(NewPlayerLeftGameEvent(mg.ID, mg.GameID, playerID, reason))
	...
	return NewPlayerNotInGameError(playerID, mg.GameID)
}
...
func (mg *MinigameAggregate) UpdatePlayerScore(playerID uint64, score int64, scoreType ScoreType) error {
	...
	player.UpdateScore(score, scoreType)
	...
	// 发布分数更新事件
	mg.addEvent(NewPlayerScoreUpdatedEvent(mg.ID, mg.GameID, playerID, oldScore, score, scoreType))
...
func (mg *MinigameAggregate) UpdateGameData(key string, value interface{}) error {
	...
	// 发布数据更新事件
	mg.addEvent(NewGameDataUpdatedEvent(mg.ID, mg.GameID, key, value))
...
func (mg *MinigameAggregate) PauseGame() error {
	...
	// 发布游戏暂停事件
	mg.addEvent(NewGamePausedEvent(mg.ID, mg.GameID))
...
func (mg *MinigameAggregate) ResumeGame() error {
	...
	// 发布游戏恢复事件
	mg.addEvent(NewGameResumedEvent(mg.ID, mg.GameID))
...
func (mg *MinigameAggregate) checkSaveDogEndConditions() {
	...
	// 检查救援进度
	if progress, ok := mg.GameData.GetData("rescue_progress").(int); ok && progress >= 100 {
	...
	// 检查时间限制
	if mg.StartTime != nil {
		if timeLimit, ok := mg.GameData.GetData("time_limit").(int); ok {
...
func (mg *MinigameAggregate) checkPuzzleEndConditions() {
	...
	// 检查完成的拼图块数
	completedPieces, _ := mg.GameData.GetData("completed_pieces").(int)
	totalPieces, _ := mg.GameData.GetData("total_pieces").(int)
...
func (mg *MinigameAggregate) calculateFinalResults(reason GameEndReason) {
	...
	for i, player := range players {
		result := &GameResult{
			...
			Rank:        int32(i + 1),
			Score:       player.Score,
			IsWinner:    i == 0 && reason == GameEndReasonCompleted,
			EndReason:   reason,
			PlayTime:    mg.Duration,
			Timestamp:   time.Now(),
		}
