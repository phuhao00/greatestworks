...
func healthCheck(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		logger.Error("[healthCheck] Write err:%v", err.Error())
	}
}
...
func setLogLevel(w http.ResponseWriter, r *http.Request) {
	...
	err := r.ParseForm()
	if err != nil {
		logger.Error("[setLogLevel] param err:%v", err.Error())
		return
	}
	...
	if level == "DEBUG" {
		...
		logger.Warn("[setLogLevel] The server log level could not be set as %v.", level)
		return
	}
	_, err = w.Write([]byte(tips))
	if err != nil {
		logger.Error("[setLogLevel] Write err:%v", err.Error())
	}
}
